...
function buildDeterministicSuggestion(summary, mode = 'balanced', householdProfile) {
  ...
  if (available <= 0) return { source: 'deterministic', mode: 'Balanserad budget', explanation: 'Klirr hittar inget utrymme efter fasta fasta utgifter.', buffer: 0, marginLeft: 0, safetyTotal: 0, householdProfile: profile, categoryCaps: caps, clampedCategories: [], overflowToSafety: 0, guidelineComparison: { food: compareFoodGuideline(getFoodReferenceAmount(profile), 0) }, items: labels.map(([label, category]) => ({ label, category, amount: 0 })), warning: 'Det finns inget positivt utrymme att fördela just nu.', nextStep: 'Granska Fasta utgifter och inkomster först.' };
