# Klirr Product Principles: Budget First

## Syfte

Klirr hjälper användaren att bygga, förstå och skydda en hållbar **Budget** baserad på vad livet kostar varje månad just nu. Produkten ska beskriva ett återkommande ekonomiskt tillstånd, inte göra användaren till bokförare av historiska månader.

## Budgeten är produkten

Budgeten är Klirrs centrala produktkoncept: återkommande inkomster, Måsten, återkommande kostnader, Rörlig Budget och Marginal i ett sammanhållet system. Kalender­månaden är bara en beräkningsenhet.

## Återkommande kostnader är sanningen

Klirr ska prioritera poster som fortsätter påverka Budgeten framåt. Importerade transaktioner är underlag för att hitta återkommande inkomster och kostnader, men de blir inte Budgetens sanning förrän användaren har bekräftat dem.

## Budget Buddy

Budget Buddy finns för att skydda och förbättra användarens Budget. Buddy ska förklara hur ändringar påverkar hållbarhet, marginal, återkommande inkomster och återkommande kostnader. Buddy får föreslå åtgärder, men ska inte påstå att Budgeten förbättrats innan användaren har bekräftat och åtgärden har applicerats.

## Vad Klirr inte är

Klirr är inte främst:

- en historisk spending tracker
- en månad-för-månad-jämförelse
- en transaktionsdagbok
- en bankbalans-app
- en mätare av rikedom eller personligt värde

## Feature test

1. Does this help the user understand what their life costs every month right now?
2. Does this help the user build or protect a sustainable Budget?
3. Does this improve clarity around recurring income, recurring costs, variable planning, or margin?
4. Is this about the current financial state, or is it drifting into historical bookkeeping?
5. Would this feature still make sense without comparing calendar months?

If a proposed feature fails these questions, it probably does not belong in Klirr or needs reframing.


## Onboarding är en guide, inte ett krav

Användaren kan alltid hoppa in i Klirr på sitt sätt: bygga första Budgeten steg för steg, utforska själv eller importera en Budget. Onboarding ska kunna hoppas över, pausas, återupptas och byta riktning utan att låsa appen. Alla kärnfunktioner ska vara tillgängliga även när setup är ofullständig.

Klirr ska aldrig straffa, skamma eller skuldbelägga en ofullständig Budget. Kopian ska hjälpa användaren bygga klart Budgeten i sin egen takt och påminna om att Budget Buddy kan hjälpa när användaren vill.

## Budget First language

Preferred:

- Din Budget
- Budgethälsa
- Budget Buddy
- Budget Checkup
- Rörlig Budget
- Återkommande kostnader
- Marginal
- Hållbar Budget

Avoid when Budget is meant:

- Din ekonomi
- Din ekonomiska situation
- Månadens resultat
- Så gick månaden
- Förra månaden
- Nästa månad ser bättre ut

## Tonprinciper

Skriv varmt, tydligt och konkret. Föredra språk som visar vad Budgeten betyder framåt:

- “Ditt liv kostar just nu …”
- “Din Budget har blivit starkare …”
- “Dina återkommande kostnader har ökat …”
- “Din Budget saknar marginal …”
- “Det här gör Budgeten mer hållbar.”

Undvik att rama in produkten som en historisk månad. Säg hellre “Budgeten saknar just nu marginal” än “månaden gick dåligt”.
