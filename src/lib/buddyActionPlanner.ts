...
function makeRecurringAction(message: string, context: any): BuddyActionPlan | null {
  ...
  if (/(ska inte vara ett måste|ska inte vara ett maste|flytta .*rörlig|flytta .*rorlig)/i.test(message)) {
    return { intent: 'move_recurring_item', confidence: 'high', proposedAction: { id: uid('buddy_action'), type: 'move_recurring_item', title: 'Flytta post', description: `Flytta “${item.label}” från Måsten till Rörlig Budget. Kontoutdraget ändras inte.`, payload: { recurringId: item.id, label: item.label, from: 'fixed', to: 'variable', category: item.category }, confirmLabel: 'Ja, flytta posten', cancelLabel: 'Nej, låt den vara', status: 'pending', riskLevel: 'low', undoable: true } };
  }
...
export function buildBudgetCheckupIssues(context: any): BudgetCheckupIssue[] {
  ...
  if (remainingAfterPlan < 0) add({ label: `Planen går minus med ${formatKr(Math.abs(remainingAfterPlan))}`, severity: 'danger', nextAction: 'Minska Rörlig Budget eller granska Måsten först.', tab: 'plan', message: 'Gör en tryggare Rörlig Budget' });
  else if (totalIncome > 0 && remainingAfterPlan <= Math.max(500, totalIncome * 0.02)) add({ label: `Marginalen efter planen är nära noll (${formatKr(remainingAfterPlan)})`, severity: 'warning', nextAction: 'Lämna lite luft i planen så oväntade köp inte spräcker månaden.', message: 'Gör en tryggare Rörlig Budget' });
  if (remainingAfterFixed > 0 && variablePlanTotal >= remainingAfterFixed * 0.98) add({ label: 'Rörlig Budget äter upp nästan allt som finns kvar efter Måsten', severity: 'warning', nextAction: 'Be Buddy göra en tryggare plan med buffert och marginal.', message: 'Gör en tryggare Rörlig Budget' });
  ...
  if (!crisis && variablePlan.length && buffer === 0) add({ label: 'Buffert/sparande är 0 kr', severity: 'warning', nextAction: 'I trygg/balanserad budget bör en liten buffert prioriteras.', tab: 'variablePlan', message: 'Gör en tryggare Rörlig Budget med buffert' });
  ...
  if (food > 0) {
    ...
    if (remainingAfterFixed > 0 && food >= remainingAfterFixed * 0.76) add({ label: 'Mat tar över 75–80% av pengarna som finns kvar efter Måsten', severity: 'warning', nextAction: 'Säkra att även transport, buffert och övrigt får plats.', message: 'Gör en tryggare Rörlig Budget' });
  }
  ...
  if (!variablePlan.length) add({ label: 'Rörlig plan saknas / Rörlig Budget saknas', severity: 'warning', nextAction: 'Skapa en plan för mat, transport, nöje, övrigt och buffert.', message: 'Lägg upp en ny Rörlig Budget' });
