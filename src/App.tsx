...
import { buddySuggestionItems, initialBuddyMessage, makeBuddyReply, type BuddySuggestion, type BuddySuggestionGroup } from './lib/budgetBuddy';
...
import { buddyCapabilityInfo, buddyCapabilityTier, classifyBuddyRequest, hasBuddyCapability, premiumPreviewText, type BuddyCapability } from './lib/budgetBuddyAccess';
...
import { buildPremiumValueSummary } from './lib/premiumValue';
...
const nav: Array<{ id: TabId; label: string; shortLabel: string; icon: string }> = [
  ...
  { id: 'premium', label: 'Premium', shortLabel: 'Premium', icon: '💎' },
...
type PendingBuddyRequest = { id: string; text: string; capability: BuddyCapability; source: 'composer' | 'quick_choice' | 'handoff' | 'action'; userMessageId?: string; requestedAt: string; planGateReason?: string; aiGateReason?: string; resumed?: boolean };
type BuddySendOptions = { capability?: BuddyCapability; source?: PendingBuddyRequest['source']; appendUserMessage?: boolean; pending?: PendingBuddyRequest; forceLocal?: boolean };
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main">
        ...
        {!showOnboarding && appAccessible && tab === 'premium' && <PremiumView state={state} setState={setState} summary={summary} detection={detection} budgetHealth={budgetHealth} visibleReviewCount={visibleReviewItems.length} possibleIncomeDuplicates={possibleIncomeDuplicates} setTab={selectTab} onBuddyMessage={(message) => { setBuddyAutoMessage(message); selectTab('buddy'); }} />}
...
function DashboardView({ summary, budgetHealth, budgetCompletion, detection, visibleReviewCount, loadDemo, setTab, hasData, onboarding, dismissFirstRunGuide, onExport, onImproveBudgetHealth, premiumValue }: { summary: ReturnType<typeof calculateBudget>; budgetHealth: ReturnType<typeof calculateBudgetHealth>; budgetCompletion: ReturnType<typeof calculateBudgetCompletion>; detection: DetectionResult; visibleReviewCount: number; loadDemo: () => void; setTab: (t: TabId) => void; hasData: boolean; onboarding?: AppState['onboarding']; dismissFirstRunGuide: () => void; onExport: () => void; onImproveBudgetHealth: () => void; premiumValue: ReturnType<typeof buildPremiumValueSummary> }) {
...
function PremiumView({ state, setState, summary, detection, budgetHealth, visibleReviewCount, possibleIncomeDuplicates, setTab, onBuddyMessage }: { state: AppState; setState: (s: AppState) => void; summary: ReturnType<typeof calculateBudget>; detection: DetectionResult; budgetHealth: ReturnType<typeof calculateBudgetHealth>; visibleReviewCount: number; possibleIncomeDuplicates: ReturnType<typeof detectPossibleIncomeDuplicates>; setTab: (t: TabId) => void; onBuddyMessage: (message: string) => void }) {
  const isPremium = state.subscriptionPlan === 'pro';
  const value = buildPremiumValueSummary({ state, summary, detection, visibleReviewCount, duplicateIncomeCount: possibleIncomeDuplicates.length });
  const workspaceId = state.activeWorkspaceId || 'local-demo-workspace';
  function activatePremium() {
    const firstActivatedAt = state.premiumActivation?.firstActivatedAt || new Date().toISOString();
    setState({ ...state, subscriptionPlan: 'pro', subscriptionStatus: 'active', entitlements: getEntitlements('pro'), premiumActivation: { firstActivatedAt, onboardingShown: true } });
  }
  function switchFree() { setState({ ...state, subscriptionPlan: 'free', subscriptionStatus: 'inactive', entitlements: getEntitlements('free') }); }
  function dismissReady() { setState({ ...state, premiumActivation: { ...(state.premiumActivation || { onboardingShown: true }), onboardingShown: true, dismissedAt: new Date().toISOString() } }); }
  function createSuggestedGoal() {
    const goal = { id: uid('goal'), workspaceId, title: value.suggestedFirstGoal, reason: value.topOpportunity?.explanation || 'Föreslaget från din nuvarande Budget.', targetType: value.suggestedFirstGoal.includes('10') ? 'margin_ratio' as const : value.suggestedFirstGoal.includes('oklara') ? 'review_done' as const : value.suggestedFirstGoal.includes('noll') ? 'margin_ratio' as const : 'budget_health' as const, targetValue: value.suggestedFirstGoal.includes('10') ? 0.1 : value.suggestedFirstGoal.includes('oklara') ? 0 : value.suggestedFirstGoal.includes('noll') ? 0 : Math.min(100, budgetHealth.score + 8), currentValue: value.suggestedFirstGoal.includes('10') ? (summary.totalIncome > 0 ? summary.remainingAfterPlan / summary.totalIncome : 0) : value.suggestedFirstGoal.includes('oklara') ? visibleReviewCount : value.suggestedFirstGoal.includes('noll') ? summary.remainingAfterPlan : budgetHealth.score, status: 'active' as const, createdAt: new Date().toISOString(), nextStep: value.topOpportunity?.suggestedAction, destination: value.topOpportunity?.destination || 'dashboard' as TabId };
    setState({ ...state, premiumGoals: [...(state.premiumGoals || []).filter(g => g.workspaceId === workspaceId || g.workspaceId !== workspaceId), goal] });
  }
  function saveSnapshot() {
    const snapshot = { id: uid('snap'), workspaceId, createdAt: new Date().toISOString(), totalIncome: summary.totalIncome, fixedTotal: summary.fixedTotal, variablePlanTotal: summary.variablePlanTotal, remainingAfterPlan: summary.remainingAfterPlan, budgetHealthScore: budgetHealth.score, note: 'Sparat från Min utveckling.' };
    setState({ ...state, premiumSnapshots: [...(state.premiumSnapshots || []).filter(s => s.workspaceId === workspaceId), snapshot].slice(-12) });
  }
  function runMonitoring() {
    setState({ ...state, premiumMonitoring: { enabled: true, lastRunAt: new Date().toISOString(), nextRunHint: 'Nästa gång Klirr öppnas eller när du väljer Kör nu.', dismissedFingerprints: state.premiumMonitoring?.dismissedFingerprints || [] } });
  }
  const goals = (state.premiumGoals || []).filter(g => g.workspaceId === workspaceId);
  const snapshots = (state.premiumSnapshots || []).filter(s => s.workspaceId === workspaceId);
  return <><PageTitle title="Premium" subtitle="Förbättra Budgeten. Följ resultatet. Låt Klirr hålla koll." />
    {!isPremium && <Card className="soft"><h3>Du kan skapa, förstå och underhålla hela din Budget gratis.</h3><p className="hint">Premium lägger till en personlig förbättringsplan, utveckling över tid och lokal koll — utan att låsa din kärn-Budget.</p><div className="grid grid-3 compact-grid"><MetricCard label="Möjligheter hittade" value={String(value.opportunityCount)} /><MetricCard label="Uppskattad möjlig marginaleffekt" value={`${fmt(value.estimatedMonthlyMarginPotential)} /mån`} /><MetricCard label="Första mål" value={value.suggestedFirstGoal} /></div><div className="grid grid-3 compact-grid"><Card><h3>Förbättra min Budget</h3><p>{value.topOpportunity?.title || 'Hitta nästa bästa steg.'}</p></Card><Card><h3>Följ min utveckling</h3><p>{value.developmentSummary}</p></Card><Card><h3>Klirr håller koll</h3><p>{value.monitoringSummary}</p></Card></div><div className="row"><button className="btn primary" onClick={activatePremium}>Aktivera Premium i demon</button><span className="hint">Ingen betalning genomförs.</span></div></Card>}
    {isPremium && !state.premiumActivation?.dismissedAt && <Card className="soft"><h3>Premium är redo 💎</h3><p>Klirr har analyserat din nuvarande Budget lokalt. Uppskattningarna är inte garantier.</p><div className="grid grid-3 compact-grid"><MetricCard label="Förbättringsmöjligheter" value={String(value.opportunityCount)} /><MetricCard label="Uppskattad möjlig effekt" value={`${fmt(value.estimatedMonthlyMarginPotential)} /mån`} /><MetricCard label="Områden att granska" value={String(value.reviewAreaCount)} /></div><div className="row"><button className="btn primary" onClick={() => document.getElementById('premium-plan')?.scrollIntoView({ behavior: 'smooth' })}>Visa min förbättringsplan</button><button className="btn" onClick={createSuggestedGoal}>Skapa föreslaget mål</button><button className="btn" onClick={runMonitoring}>Aktivera automatisk koll</button><button className="btn ghost" onClick={dismissReady}>Inte nu</button></div></Card>}
    {isPremium && <><Card><h3>Liveöversikt</h3><div className="grid grid-3 compact-grid"><MetricCard label="Förbättringsmöjligheter" value={`${value.opportunityCount} hittade`} /><MetricCard label="Uppskattad möjlig marginalförbättring" value={`${fmt(value.estimatedMonthlyMarginPotential)} /mån`} /><MetricCard label="Budgethälsa kan förbättras" value={`+${value.estimatedBudgetHealthPotential} steg`} /><MetricCard label="Toppåtgärd" value={value.topOpportunity?.title || 'Behåll nuläget'} /><MetricCard label="Mål" value={value.currentGoalSummary} /><MetricCard label="Koll" value={value.monitoringSummary} /></div><div className="row"><button className="btn primary" onClick={() => document.getElementById('premium-plan')?.scrollIntoView({ behavior: 'smooth' })}>Visa min förbättringsplan</button><button className="btn" onClick={() => document.getElementById('premium-development')?.scrollIntoView({ behavior: 'smooth' })}>Se min utveckling</button><button className="btn" onClick={() => document.getElementById('premium-monitoring')?.scrollIntoView({ behavior: 'smooth' })}>Hantera automatisk koll</button><button className="btn ghost" onClick={switchFree}>Växla till Gratis i demon</button></div></Card>
    <div id="premium-plan"><Card><h3>Min förbättringsplan</h3><p className="hint">Uppskattning baserad på din nuvarande Budget. Inte en garanterad besparing och inget ändras utan bekräftelse.</p><div className="stack">{value.opportunities.map((item, idx) => <div className="list-line" key={item.id}><span><b>{idx + 1}. {item.title}</b><br/><small>{item.explanation}</small><br/><small>{item.estimationNote}</small></span><span><b>{fmt(item.estimatedMarginImpact)} /mån</b><br/><small>Budgethälsa +{item.estimatedBudgetHealthImpact} · {item.confidence}</small><br/><button className="btn small" onClick={() => setTab(item.destination)}>{item.suggestedAction}</button>{item.suggestedBuddyMessage && <button className="btn small" onClick={() => onBuddyMessage(item.suggestedBuddyMessage!)}>Buddy+</button>}</span></div>)}</div>{!value.opportunities.length && <Empty><b>Inga akuta förbättringar hittades.</b><br/>Spara ett nuläge och låt Klirr hålla koll på förändringar framåt.</Empty>}<h4>Tre alternativa planer</h4><div className="grid grid-3 compact-grid">{value.alternatives.map(alt => <Card key={alt.id}><h3>{alt.label}</h3><p>{alt.summary}</p><p className="hint">Estimerad marginal efter plan: <b>{fmtSigned(alt.estimatedMargin)}</b>. Budgethälsa {alt.estimatedHealthDirection}.</p><ul>{alt.tradeoffs.map(t => <li key={t}>{t}</li>)}</ul><button className="btn" onClick={() => onBuddyMessage(`Visa ${alt.label.toLowerCase()} förbättringsplan som actionkort. Ändra inget utan bekräftelse.`)}>Förbered actionkort</button></Card>)}</div></Card></div>
    <div id="premium-development"><Card><h3>Min utveckling</h3><p>{value.developmentSummary}</p>{value.strongestPositiveChange && <p className="pill green">{value.strongestPositiveChange}</p>}{value.strongestNegativeChange && <p className="pill warn">{value.strongestNegativeChange}</p>}<div className="grid grid-3 compact-grid"><MetricCard label="Nuvarande marginal" value={fmtSigned(summary.remainingAfterPlan)} /><MetricCard label="Fasta utgifter" value={fmt(summary.fixedTotal)} /><MetricCard label="Budgethälsa" value={`${budgetHealth.score}%`} /></div><button className="btn" onClick={saveSnapshot}>Spara nuläge</button>{snapshots.length > 0 && <details><summary>Historik som sekundär detalj</summary><div className="stack">{snapshots.slice().reverse().map(s => <div className="list-line" key={s.id}><span>{new Date(s.createdAt).toLocaleString('sv-SE')}</span><b>{fmtSigned(s.remainingAfterPlan)} · {s.budgetHealthScore}%</b></div>)}</div></details>}</Card></div>
    <Card><h3>Mål</h3><p className="hint">Rekommenderat mål: {value.suggestedFirstGoal}. Skapas bara när du bekräftar.</p><button className="btn" onClick={createSuggestedGoal}>Skapa föreslaget mål</button><div className="stack">{goals.map(goal => <div className="list-line" key={goal.id}><span><b>{goal.title}</b><br/><small>{goal.reason}</small><br/><small>Nästa steg: {goal.nextStep || 'Fortsätt enligt planen'}</small></span><span>{goal.currentValue.toLocaleString('sv-SE')} / {goal.targetValue.toLocaleString('sv-SE')}<br/>{goal.destination && <button className="btn small" onClick={() => setTab(goal.destination!)}>Gå dit</button>}</span></div>)}</div></Card>
    <div id="premium-monitoring"><Card><h3>Klirr håller koll</h3><p>Kontrollen körs lokalt när Klirr öppnas eller när du väljer Kör nu.</p><div className="grid grid-3 compact-grid"><MetricCard label="Senaste koll" value={state.premiumMonitoring?.lastRunAt ? new Date(state.premiumMonitoring.lastRunAt).toLocaleString('sv-SE') : 'Inte körd ännu'} /><MetricCard label="Nästa koll" value={state.premiumMonitoring?.nextRunHint || 'När Klirr öppnas'} /><MetricCard label="Fynd" value={`${value.reviewAreaCount} områden`} /></div><div className="stack">{value.opportunities.map(item => <div className="list-line" key={`monitor-${item.id}`}><span><b>{item.title}</b><br/><small>Varför: {item.explanation}</small></span><button className="btn small" onClick={() => setTab(item.destination)}>{item.suggestedAction}</button></div>)}</div><button className="btn primary" onClick={runMonitoring}>Kör lokal koll nu</button></Card></div>
    <Card><h3>Mer kontroll</h3><div className="grid grid-3 compact-grid"><Card><h3>Flera Budgetar</h3><p>Separera privat Budget, företag, flytt eller andra livssituationer.</p><span className="pill">Demo-workspace: {workspaceId}</span></Card><Card><h3>Delad Budget-demo</h3><p>Förbered en gemensam Budget och roller. Demo – ingen realtidsdelning.</p></Card><Card><h3>Versioner</h3><p>Ångra större förändringar och återställ en tidigare Budget via sparade nulägen.</p></Card></div></Card></>}
  </>;
}
...
function BudgetBuddyView({ state, setState, summary, detection, visibleReviewCount, handledReviewCount, possibleIncomeDuplicates, setTab, setScenarioOff, autoMessage, onAutoMessageHandled }: { state: AppState; setState: (s: AppState) => void; summary: ReturnType<typeof calculateBudget>; detection: DetectionResult; visibleReviewCount: number; handledReviewCount: number; possibleIncomeDuplicates: ReturnType<typeof detectPossibleIncomeDuplicates>; setTab: (t: TabId) => void; setScenarioOff: (ids: string[]) => void; autoMessage?: string; onAutoMessageHandled?: () => void }) {
  ...
  const [pendingBuddyRequest, setPendingBuddyRequest] = useState<PendingBuddyRequest | null>(null);
  ...
  useEffect(() => {
    if (!pendingBuddyRequest?.resumed || buddyBusy) return;
    const pending = pendingBuddyRequest;
    setPendingBuddyRequest(null);
    void send(pending.text, { capability: pending.capability, source: pending.source, appendUserMessage: false, pending });
  }, [pendingBuddyRequest?.id, pendingBuddyRequest?.resumed, buddyBusy, state.entitlements, state.privacyPreferences, state.consentRecords]);
  ...
  function buildLocalBuddyReply(text: string, baseState = state): ChatMessage {
    const localContext = { summary, detection, rules: baseState.rules, transactionCount: baseState.transactions.length };
    const reply = makeBuddyReply(text, localContext);
    const localPlan = planBuddyAction({ message: text, context: localContext, incomes: baseState.incomes, variablePlan: baseState.variablePlan, householdProfile: baseState.householdProfile });
    return { ...reply, content: `Lokalt läge

${reply.content}`, proposedAction: localPlan.proposedAction || reply.proposedAction };
  }
  function premiumPreviewMessage(pending: PendingBuddyRequest): ChatMessage {
    const details = buddyCapabilityInfo(pending.capability);
    return { id: uid('msg'), role: 'assistant', createdAt: todayIso(), content: `Premiumförhandsvisning · ${details.label}

${premiumPreviewText(pending.capability)}

Kort förhandsvisning utifrån din Budget: marginalen är ${fmtSigned(summary.remainingAfterPlan)} och Budgethälsan kan prioriteras utan att något ändras automatiskt.

Planåtkomst och AI-samtycke är separata beslut. Premium aktiverar inte AI.`, actions: [
      { label: 'Aktivera Premium i demon', message: `__activate_premium__:${pending.id}` },
      { label: 'Fortsätt med grundläggande Buddy', message: `__continue_local__:${pending.id}` },
    ] };
  }
  function aiActivationMessage(pending: PendingBuddyRequest): ChatMessage {
    return { id: uid('msg'), role: 'assistant', createdAt: todayIso(), content: `Aktivera Budget Buddy AI ✨

Klirr kan skicka en sammanfattad Budgetkontext till AI för ett mer fritt svar. Råa importerade transaktionsrader skickas inte. Du kan granska “Vad AI såg” efteråt och stänga av AI igen. Budget Buddy-kärnan fungerar även utan AI.`, actions: [
      { label: 'Aktivera Budget Buddy', message: `__activate_ai__:${pending.id}` },
      { label: 'Fortsätt utan AI', message: `__continue_local__:${pending.id}` },
      { label: 'Läs hur AI används', message: `__read_ai_info__:${pending.id}` },
    ] };
  }
  function resumePending(pending: PendingBuddyRequest) {
    if (pending.resumed) return;
    setPendingBuddyRequest({ ...pending, resumed: true });
  }
  async function send(text: string, options: BuddySendOptions = {}) {
    const trimmed = text.trim();
    if (!trimmed || buddyBusy) return;
    if (trimmed.startsWith('__activate_premium__:')) {
      const id = trimmed.split(':')[1];
      const pending = pendingBuddyRequest?.id === id ? pendingBuddyRequest : null;
      const premiumState = activatePremium(state);
      setState(premiumState);
      if (pending) resumePending(pending);
      return;
    }
    if (trimmed.startsWith('__activate_ai__:')) {
      const id = trimmed.split(':')[1];
      const pending = pendingBuddyRequest?.id === id ? pendingBuddyRequest : null;
      const next = addConsentRecord({ ...normalizePrivacyState(state), privacyPreferences: { ...(state.privacyPreferences || defaultPrivacyPreferences()), aiEnabled: true } }, { type: 'ai_features', documentVersion: legalDocumentConfig.aiInfoVersion, status: 'accepted', source: 'settings', locale: 'sv-SE' });
      setState(next);
      if (pending) resumePending(pending);
      return;
    }
    if (trimmed.startsWith('__continue_local__:')) {
      const id = trimmed.split(':')[1];
      const pending = pendingBuddyRequest?.id === id ? pendingBuddyRequest : null;
      if (pending) {
        setPendingBuddyRequest(null);
        const reply = buildLocalBuddyReply(pending.text);
        setState({ ...state, chatMessages: [...state.chatMessages, reply] });
      }
      return;
    }
    if (trimmed.startsWith('__read_ai_info__:')) {
      setTab('more');
      setState({ ...state, chatMessages: [...state.chatMessages, { id: uid('msg'), role: 'assistant', createdAt: todayIso(), content: 'Jag öppnade Mer → Sekretess & data. Ingen AI aktiverades och inget samtycke skapades.' }] });
      return;
    }
    ...
    const userMsg: ChatMessage | null = shouldAppendUser ? { id: uid('msg'), role: 'user', content: trimmed, createdAt: todayIso() } : null;
    const afterUserState = userMsg ? { ...state, chatMessages: [...state.chatMessages, userMsg] } : state;
    setDraft('');
    setQuickChoicesOpen(false);
    if (pending && intent) {
    ...
    const capability = classifyBuddyRequest(trimmed, options.capability);
    const request: PendingBuddyRequest = options.pending || { id: uid('buddy_req'), text: trimmed, capability, source: options.source || 'composer', userMessageId: userMsg?.id, requestedAt: todayIso() };
    if (!hasBuddyCapability(afterUserState.entitlements, capability)) {
      const gated = { ...request, planGateReason: 'premium_required' };
      setPendingBuddyRequest(gated);
      setState({ ...afterUserState, chatMessages: [...afterUserState.chatMessages, premiumPreviewMessage(gated)] });
      return;
    }
    const localPlan = planBuddyAction({ message: trimmed, context: { summary, detection, rules: afterUserState.rules }, incomes: afterUserState.incomes, variablePlan: afterUserState.variablePlan, householdProfile: afterUserState.householdProfile, pendingAction: pending });
    const remoteUseful = !options.forceLocal && (buddyCapabilityTier(capability) === 'advanced' || localPlan.intent === 'none' || /ai|djup|fördjup|analysera|varför/i.test(trimmed));
    if (!remoteUseful || options.forceLocal) {
      setPendingBuddyRequest(null);
      const reply = buildLocalBuddyReply(trimmed, afterUserState);
      setState({ ...afterUserState, chatMessages: [...afterUserState.chatMessages, reply] });
      return;
    }
    const safeAi = prepareSafeAiContext({ state: afterUserState, summary, detection, userMessage: trimmed, requestType: buddyCapabilityTier(capability) === 'advanced' ? 'premium_buddy_plus' : 'budget_buddy_chat', purpose: buddyCapabilityTier(capability) === 'advanced' ? 'Premium Buddy+ fortsättning' : 'Budget Buddy-fråga', visibleReviewCount, handledReviewCount, workspaceId: state.activeWorkspaceId });
    if (!safeAi.allowed) {
      const gated = { ...request, aiGateReason: safeAi.logEntry.failureReason || safeAi.reason };
      setPendingBuddyRequest(gated);
      const blockedState = appendAiLog(afterUserState, safeAi.logEntry);
      setState({ ...blockedState, chatMessages: [...blockedState.chatMessages, aiActivationMessage(gated)] });
      return;
    }
    setPendingBuddyRequest(null);
    ...
    try {
      ...
      const context = { ...safeAi.context, capability, capabilityTier: buddyCapabilityTier(capability) };
      const recentMessages = state.chatMessages.slice(-8).map(m => ({ role: m.role, content: m.content }));
      const response = await fetch('/api/budget-buddy', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: trimmed, context, recentMessages, currentDate, currentMonth: new Date(currentDate).getMonth() + 1, capability, capabilityTier: buddyCapabilityTier(capability) }) });
      const data = await response.json();
      const proposedAction = data.proposedAction || localPlan.proposedAction;
      const reply: ChatMessage = { id: uid('msg'), role: 'assistant', content: data.message || localPlan.clarificationQuestion || buildLocalBuddyReply(trimmed, afterUserState).content, createdAt: todayIso(), actions: Array.isArray(data.actions) ? data.actions as BuddyAction[] : undefined, proposedAction };
      ...
      const reply = buildLocalBuddyReply(trimmed, afterUserState);
      setState({ ...afterUserState, chatMessages: [...afterUserState.chatMessages, { ...reply, content: `${reply.content}

AI var tillfälligt otillgänglig, så jag svarade tryggt lokalt. Inga ändringar gjordes utan bekräftelse.` }] });
    } finally {
      setBuddyBusy(false);
    }
  }
  ...
  function handleSuggestion(suggestion: BuddySuggestion) {
    setQuickChoicesOpen(false);
    quickChoicesTriggerRef.current?.focus();
    void send(suggestion.message, { capability: suggestion.capability, source: 'quick_choice' });
  }
  ...
  function runAction(action: NonNullable<ChatMessage['actions']>[number]) {
    ...
    if (action.message) send(action.message, { source: 'action' });
  }
  ...
  return <Card className="chat-shell">
    <div className="chat-header"><h2 style={{ margin: 0 }}>Budget Buddy ✨</h2><p style={{ margin: '3px 0 0', color: 'var(--muted)' }}>{hasEntitlement(state.entitlements, 'budgetBuddyAdvanced') ? 'Premium · Budget Buddy+' : 'Gratis · Grundläggande Buddy'} · {state.privacyPreferences?.aiEnabled && hasAcceptedConsent(state, 'ai_features', legalDocumentConfig.aiInfoVersion) ? 'AI aktiv · sammanfattad kontext' : 'Lokalt läge'}{state.privacyPreferences?.aiEnabled && hasAcceptedConsent(state, 'ai_features', legalDocumentConfig.aiInfoVersion) ? <button className="btn small ghost" type="button" onClick={() => setTab('more')}>Se vad AI såg</button> : null}</p><p style={{ margin: '3px 0 0', color: 'var(--muted)' }}>Din Budget-kompis. Inget ändras förrän du säger ja.</p></div>
...
function activatePremium(state: AppState): AppState { return { ...state, subscriptionPlan: 'pro', subscriptionStatus: 'active', entitlements: getEntitlements('pro', 'active'), premiumActivation: { ...(state.premiumActivation || { onboardingShown: false }), firstActivatedAt: state.premiumActivation?.firstActivatedAt || new Date().toISOString() } }; }
function activateFree(state: AppState): AppState { return { ...state, subscriptionPlan: 'free', subscriptionStatus: 'inactive', entitlements: getEntitlements('free', 'inactive') }; }
function DemoSubscriptionSwitch({ state, setState }: { state: AppState; setState: (s: AppState) => void }) {
  const premium = hasEntitlement(state.entitlements, 'premiumHub');
  return <Card><h3>Demo: abonnemangsläge</h3><p>Ingen betalning genomförs. Budgetdata, Premiumdata och sekretessinställningar bevaras när du växlar.</p><div className="segmented-control" role="radiogroup" aria-label="Demo abonnemangsläge"><button type="button" className={!premium ? 'active' : ''} aria-checked={!premium} role="radio" onClick={() => setState(activateFree(state))}>Gratis</button><button type="button" className={premium ? 'active' : ''} aria-checked={premium} role="radio" onClick={() => setState(activatePremium(state))}>Premium</button></div><p className="hint">Aktuellt läge: <b>{premium ? 'Premium' : 'Gratis'}</b>.</p></Card>;
}
