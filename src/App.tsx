...
function ExpandableBudgetItem({ id, title, amount, meta, status, tone, warning, children }: ExpandableBudgetItemProps) {
  ...
  return <section className={`budget-row ${open ? 'open' : ''} ${warning ? 'has-warning' : ''}`} data-budget-row-id={id}>
    <button id={buttonId} className="budget-row-button" type="button" aria-expanded={open} aria-controls={panelId} onClick={toggle}>
      <span className="budget-row-main"><span className="budget-row-title">{title}</span>{meta && <span className="budget-row-meta">{meta}</span>}{warning && <span className="budget-row-warning">⚠ {warning}</span>}</span>
