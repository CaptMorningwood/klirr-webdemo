...
import { deleteCloudData as deleteCloudSnapshots } from './services/accountClient';
...
const { appUserId } = useAuthSession();
const [state, setState] = useState<AppState>(() => { const saved = loadState(appUserId); const onboarding = normalizeOnboardingState(saved?.onboarding, saved?.onboardingCompleted); return normalizePrivacyState(ensureWorkspaceState({ ...initialState, ...(saved || {}), entitlements: getEntitlements(saved?.subscriptionPlan || initialState.subscriptionPlan, saved?.subscriptionStatus || initialState.subscriptionStatus), onboarding, onboardingCompleted: onboarding.status === 'COMPLETED', buddyActionHistory: saved?.buddyActionHistory || [] })); });
  ...
  useEffect(() => { if (loaded) saveState(state, appUserId); }, [appUserId, state, loaded]);
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main">
        ...
        {!showOnboarding && appAccessible && tab === 'more' && <MoreView active={moreSection} setActive={setMoreSection} privacyFocus={privacyFocus} onPrivacyFocusHandled={() => setPrivacyFocus(undefined)} state={state} setState={setState} loadDemo={loadDemo} openOnboarding={() => { setTab('dashboard'); setOnboardingOpen(true); }} onReset={() => { clearState(appUserId); setState({ ...initialState, onboardingCompleted: false, onboarding: normalizeOnboardingState() }); selectTab('dashboard'); }} />}
      </main>
...
function VariablePlanView({ variablePlan, setVariablePlan, summary, householdProfile, state, setState }: { variablePlan: VariablePlanItem[]; setVariablePlan: (p: VariablePlanItem[]) => void; summary: ReturnType<typeof calculateBudget>; householdProfile?: HouseholdProfile; state: AppState; setState: (s: AppState) => void }) {
  ...
  async function makeSuggestion(nextMode = mode) {
    ...
    try {
      const token = await auth.getToken();
      if (!token) throw new Error('Session expired');
      const response = await fetch('/api/suggest-budget', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ summary, mode: nextMode, householdProfile: normalizeHouseholdProfile(householdProfile) }) });
      if (!response.ok) throw new Error('Authenticated suggestion request failed');
      const data = await response.json();
...
function PrivacyCenterView({ state, setState, onReset, privacyFocus, onPrivacyFocusHandled }: { state: AppState; setState: (s: AppState) => void; onReset: () => void; privacyFocus?: PrivacyFocus; onPrivacyFocusHandled?: () => void }) {
  const { identity, auth } = useAuthSession();
  ...
  async function runDeletion() {
    ...
    if (deleteKind === 'cloud') {
      try {
        await deleteCloudSnapshots(auth.getToken);
        return setResult('Molndatan för ditt Klirr-konto är raderad. Lokal data ändrades inte.');
      } catch {
        return setResult('Molndatan kunde inte raderas. Ingen lokal data ändrades.');
      }
    }
  ...
  return <><PageTitle title="Sekretess & data" subtitle="Förstå, exportera och radera din data. Juridiska texter är utkast och kräver slutlig granskning före produktion." />
    ...
    <details open><summary><b>Din datalagring</b></summary><Card><h3>Data- och säkerhetsstatus</h3><div className="stack"><div className="list-line"><span>Local storage</span><span className="pill">kopplad till Klirr-kontot</span></div><div className="list-line"><span>Authentication</span><span className="pill green">verifierad via {identity.provider}</span></div><div className="list-line"><span>Cloud sync</span><span className="pill">server-scope via Klirr-ID</span></div><div className="list-line"><span>AI raw-transaction protection</span><span className="pill green">verified by code path</span></div><div className="list-line"><span>Produktidentitet</span><span className="pill green">provider-neutral</span></div></div><p className="hint">Klirr visar inte påståenden om kryptering, region, full säkerhet eller regelefterlevnad när detta inte kan verifieras.</p></Card></details>
