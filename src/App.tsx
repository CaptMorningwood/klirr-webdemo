...
function OnboardingView({ initialState, state, setState, loadDemo, setTab }: { initialState: AppState; state: AppState; setState: (s: AppState) => void; loadDemo: () => void; setTab: (t: TabId) => void }) {
  ...
  return <><PageTitle title="Kom igång med Klirr" subtitle="Välj manuell budget eller import — Budget Buddy hjälper dig till en trygg första plan." />
    ...
    {step === 'musts' && <Card><h3>Måsten 📌</h3><p className="hint">Måsten är sådant som måste betalas även en dålig månad: hyra, el, lån, försäkringar och abonnemang.</p><div className="stack">{musts.map((item, i) => <div className="edit-row" key={`${item.label}-${i}`}><input className="input" value={item.label} onChange={e => setMusts(musts.map((m, idx) => idx === i ? { ...m, label: e.target.value } : m))} /><input className="input money-input" type="number" placeholder="kr/mån" value={item.amount} onChange={e => setMusts(musts.map((m, idx) => idx === i ? { ...m, amount: e.target.value } : m))} /><input className="input" value={item.category} onChange={e => setMusts(musts.map((m, idx) => idx === i ? { ...m, category: e.target.value } : m))} /><button className="btn small danger" onClick={() => setMusts(musts.filter((_, idx) => idx !== i))}>Ta bort</button></div>)}</div><button className="btn small" onClick={() => setMusts([...musts, { label: 'Ny fast kostnad', amount: '', category: 'Fast kostnad' }])}>+ Lägg till rad</button><button className="btn primary" onClick={() => patchOnboarding('variablePlan')}>Nästa</button></Card>}
    {step === 'variablePlan' && <Card><h3>Rörlig plan 💡</h3><div className="grid grid-3"><MetricCard label="Kvar efter måsten" value={fmtSigned(incomeAmount - fixedTotal)} /><MetricCard label="Föreslagen rörlig plan" value={fmt(variableTotal)} /><MetricCard label="Marginal efter plan" value={fmtSigned(incomeAmount - fixedTotal - variableTotal)} tone={incomeAmount - fixedTotal - variableTotal >= 0 ? 'good' : 'bad'} /></div><select className="select" value={mode} onChange={e => setMode(e.target.value as BudgetSuggestionMode)}><option value="safe">Trygg: mer marginal och buffert</option><option value="balanced">Balanserad: jämn fördelning</option><option value="boost">Lite friare: mer vardag och mindre marginal</option></select><div className="stack">{proposedPlan.map(item => <div className="list-line" key={item.id}><span>{item.label}<br/><small>{item.category}</small></span><b className="mono">{fmt(item.amount)}</b></div>)}</div><p className="hint">Buffert/sparande: {fmt(proposedPlan.filter(i => /spar|buffert/i.test(i.label + i.category)).reduce((s, i) => s + i.amount, 0))}. Mat: {fmt(proposedPlan.find(i => /mat/i.test(i.label))?.amount || 0)}.</p><button className="btn primary" onClick={() => applyDraft(false)}>Visa sammanfattning</button></Card>}
...
function DashboardView({ summary, detection, visibleReviewCount, loadDemo, setTab, hasData, onboarding, dismissFirstRunGuide, onExport }: { summary: ReturnType<typeof calculateBudget>; detection: DetectionResult; visibleReviewCount: number; loadDemo: () => void; setTab: (t: TabId) => void; hasData: boolean; onboarding?: AppState['onboarding']; dismissFirstRunGuide: () => void; onExport: () => void }) {
  ...
  return <>
    ...
    {onboarding?.currentStep === 'finish' && !onboarding.firstRunGuideDismissed && <Card className="soft"><div className="row" style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}><div><b>Snabbguide för din första månad</b><p className="hint">Kolla inkomster, måsten, rörlig plan och kvar efter plan. Budget Buddy kan hjälpa dig justera utan att ändra något förrän du bekräftar.</p><div className="row"><button className="btn small" onClick={() => setTab('income')}>Inkomst</button><button className="btn small" onClick={() => setTab('musts')}>Måsten</button><button className="btn small" onClick={() => setTab('variablePlan')}>Rörlig plan</button><button className="btn small primary" onClick={() => setTab('buddy')}>Budget Buddy ✨</button></div></div><button className="btn small ghost" onClick={dismissFirstRunGuide}>Stäng</button></div></Card>}
    ...
    <div className="grid grid-2" style={{ marginTop: 16 }}>
      ...
      <Card><h3>Nästa steg</h3><div className="stack"><button className="btn" onClick={() => setTab('review')}>Granska oklara poster ({visibleReviewCount})</button><button className="btn" onClick={() => setTab('recurring')}>Bekräfta återkommande</button><button className="btn" onClick={onExport}>Exportera rapport</button></div></Card>
    </div>
  </>;
}
...
Obs: AI-endpointen kunde inte nås, så detta är lokalt fallback-svar.` }] });
  ...
  function actionSummary(action: BuddyProposedAction) {
    ...
    if (action.type === 'update_variable_plan') {
      ...
      return <div className="stack">{action.payload.mode === 'crisis' && <p className="hint">🛟 Tillfälligt krisläge: mat skyddas först; nöje, sparande och marginal sänks innan mat blir orimligt låg. Granska manuellt.</p>}<div className="list-line"><span>Nuvarande rörlig plan</span><b className="mono">{fmt(currentTotal)}</b></div><div className="list-line"><span>Föreslagen rörlig plan</span><b className="mono">{fmt(total)}</b></div>{action.payload.items.map(item => <div className="list-line" key={item.id || item.label}><span>{item.label}<br/><small>{item.category}</small></span><b className="mono">{fmt(item.amount)}</b></div>)}{Number.isFinite(action.payload.marginLeft) && <div className="list-line"><span>Uppskattat kvar efter plan</span><b className="mono">{fmt(action.payload.marginLeft || 0)}</b></div>}<p className="hint">Det här ersätter den nuvarande rörliga planen först när du bekräftar.</p>{action.payload.notes && <p className="hint">{action.payload.notes}</p>}{riskText && <span className="pill">{riskText}</span>}</div>;
    }
