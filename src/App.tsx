...
function DashboardView({ summary, budgetHealth, budgetCompletion, detection, visibleReviewCount, loadDemo, setTab, hasData, onboarding, dismissFirstRunGuide, onExport, onImproveBudgetHealth }: { summary: ReturnType<typeof calculateBudget>; budgetHealth: ReturnType<typeof calculateBudgetHealth>; budgetCompletion: ReturnType<typeof calculateBudgetCompletion>; detection: DetectionResult; visibleReviewCount: number; loadDemo: () => void; setTab: (t: TabId) => void; hasData: boolean; onboarding?: AppState['onboarding']; dismissFirstRunGuide: () => void; onExport: () => void; onImproveBudgetHealth: () => void }) {
  ...
  return <>
    ...
    <Card className="budget-list-card"><div className="budget-list">
      ...
      {row('musts', 'Måsten / återkommande kostnader', fmt(summary.fixedTotal), summary.totalIncome > 0 ? `${pct(fixedPct)} av inkomsten · ${musts.text}` : musts.text, <><p>Måsten är kostnader som påverkar Budgeten framåt.</p><div className="progress"><div style={{ width: `${Math.min(100, fixedPct)}%` }} /></div>{summary.fixedItems.slice(0, 6).map(i => <div className="list-line" key={i.id}><span>{i.label}</span><b className="mono">{fmt(i.amount)}</b></div>)}<button className="btn small" onClick={() => setTab('musts')}>Visa Måsten</button></>, fixedPct > 65, fixedPct > 80 ? 'danger' : 'warn', <MiniProgressBar value={musts.width} label={summary.totalIncome > 0 ? `Måsten är ${Math.round(musts.share)} procent av inkomsten` : musts.text} tone={fixedPct > 80 ? 'danger' : fixedPct > 65 ? 'warn' : 'green'} />)}
      {row('variable', 'Rörlig Budget', fmt(summary.variablePlanTotal), variableDistribution.total > 0 && variableDistribution.largest ? `Störst: ${variableDistribution.largest.label} ${Math.round(variableDistribution.largest.share)} %` : `${summary.variableItems.length} kategorier`, <><p>Planerat utrymme för vardagliga och flexibla delar.</p>{summary.variableItems.slice(0, 6).map(i => <div className="list-line" key={i.id}><span>{i.label}</span><b className="mono">{fmt(i.amount)}</b></div>)}<p className="hint">Buffert/sparande: {fmt(buffer)}. Mat: {fmt(food)}.</p><button className="btn small" onClick={() => setTab('variablePlan')}>Visa Rörlig Budget</button></>, false, undefined, variableDistribution.total > 0 ? <SegmentedBudgetBar segments={variableDistribution.segments} label={`Rörlig Budget: ${variableDistribution.segments.map(segment => `${segment.label} ${Math.round(segment.share)} procent`).join(', ')}`} /> : undefined)}
      {row('margin', 'Marginal', fmtSigned(summary.remainingAfterPlan), `${marginVisual.text} · ${pct(marginRatio)} av inkomsten`, <><p>Marginal är det som finns kvar efter Måsten och Rörlig Budget. Den visar hur mycket Budgeten tål innan den blir sårbar.</p><p>Marginal som andel av inkomsten: <b>{pct(marginRatio)}</b>.</p><div className="row"><button className="btn small" onClick={() => setTab('variablePlan')}>Justera Rörlig Budget</button><button className="btn small" onClick={onImproveBudgetHealth}>Fråga Buddy</button></div></>, summary.remainingAfterPlan < summary.totalIncome * 0.05, summary.remainingAfterPlan < 0 ? 'danger' : 'warn', <MiniProgressBar value={marginVisual.width} label={`${marginVisual.text}, ${Math.round(marginVisual.share)} procent av inkomsten`} tone={marginVisual.state === 'negative' ? 'danger' : marginVisual.state === 'thin' ? 'warn' : 'green'} />)}
      {row('completion', 'Budgetkomplettering', `${budgetCompletion.percentage}%`, completionLabel, <><p>Budgetkomplettering mäter setup-täckning. Budgethälsa mäter hållbarhet.</p><ul>{budgetCompletion.items.map(item => <li key={item.key}>{item.completed ? 'Klart:' : 'Saknas:'} {item.label}</li>)}</ul></>)}
...
function MustsView({ summary, detection, state, setState }: { summary: ReturnType<typeof calculateBudget>; detection: DetectionResult; state: AppState; setState: (s: AppState) => void }) {
  ...
  return <><PageTitle title="Återkommande kostnader" subtitle="Fasta kostnader som måste betalas varje månad. Lägg manuella måsten här, inte under inkomster." />
    <CompactSummary items={[{ label: 'Måsten per månad', value: fmt(summary.fixedTotal) }, { label: 'Aktiva poster', value: String(manualMusts.filter(m => m.active).length + recurringMusts.filter(r => (state.recurringDecisions[r.id]?.status || (r.confidence >= 70 ? 'confirmed' : 'pending')) !== 'rejected').length) }]} />
    <Card className="budget-list-card"><div className="row" style={{ justifyContent: 'space-between', marginBottom: 10 }}><div><h3>Överblick över måsten</h3><p className="hint">Importerade måsten redigeras som overrides. Kontoutdraget ändras inte.</p></div></div><div className="budget-list">
      ...
      {!hasMusts && <Empty><b>Inga måsten ännu.</b><br/>Lägg till återkommande kostnader som hyra, el och försäkring så Klirr kan visa vad livet kostar varje månad.</Empty>}
    </div></Card>
    <Card><h3>Lägg till Måste</h3><div className="row"><input className="input" placeholder="Namn" value={label} onChange={e => setLabel(e.target.value)} /><input className="input money-input" type="number" placeholder="kr/mån" value={amount} onChange={e => setAmount(e.target.value)} /><input className="input category-input" placeholder="Kategori" value={category} onChange={e => setCategory(e.target.value)} /><button className="btn primary" onClick={add}>Lägg till</button></div></Card>
  </>;
}
