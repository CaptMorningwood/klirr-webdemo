...
function DashboardView({ summary, detection, visibleReviewCount, loadDemo, setTab, hasData, onExport }: { summary: ReturnType<typeof calculateBudget>; detection: DetectionResult; visibleReviewCount: number; loadDemo: () => void; setTab: (t: TabId) => void; hasData: boolean; onExport: () => void }) {
  if (!hasData) {
...
Obs: AI-endpointen kunde inte nås, så detta är lokalt fallback-svar.` }] });
  ...
  function actionSummary(action: BuddyProposedAction) {
    ...
    if (action.type === 'run_budget_checkup') return <div className="stack">{action.payload.issues.map((issue, i) => <div className="list-line" key={i}><span>{issue.label}<br/><small>{issue.nextAction}</small></span><span className="pill">{issue.severity}</span></div>)}</div>;
