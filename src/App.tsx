...
function OnboardingView({ initialState, state, setState, loadDemo, setTab }: { initialState: AppState; state: AppState; setState: (s: AppState) => void; loadDemo: () => void; setTab: (t: TabId) => void }) {
  ...
  function pause(currentStep: OnboardingStep = step) { setState({ ...state, onboarding: { ...onboarding, started: true, currentStep, status: onboarding.status === 'IMPORT_PATH' ? 'IMPORT_PATH' : onboarding.path === 'import' ? 'IMPORT_PATH' : 'MANUAL_PATH' } }); setTab('dashboard'); }
  ...
  return <><PageTitle title="Kom igång med Klirr" subtitle="Välj manuell budget eller import — Budget Buddy hjälper dig till en trygg första plan." />
    ...
    {step === 'import' && <Card><h3>Importera kontoutdrag</h3><p className="hint">Klirr skickar inte banktransaktioner till OpenAI. Importen sker lokalt och onboarding fortsätter efter filen är importerad.</p><div className="row" style={{ marginTop: 12 }}><button className="btn primary" onClick={() => { setState({ ...state, householdProfile: profile, onboarding }); setTab('import'); }}>Öppna Import & granskning</button><PauseButton /></div></Card>}
...
function MoreView({ active, setActive, state, setState, onReset, loadDemo }: { active: MoreSection; setActive: (s: MoreSection) => void; state: AppState; setState: (s: AppState) => void; onReset: () => void; loadDemo: () => void }) {
  return <><PageTitle title="Mer / Inställningar" subtitle="Regler, export, sync, demo-data och inställningar." />
    ...
    {active === 'settings' && <SettingsView state={state} setState={setState} loadDemo={loadDemo} onReset={onReset} restartOnboarding={() => setState({ ...state, onboardingCompleted: false, onboarding: { ...normalizeOnboardingState(state.onboarding), status: 'MANUAL_PATH', path: 'manual', started: true, currentStep: 'household' } })} />}
  </>;
}
  ...
  useEffect(() => {
    ...
    if (ob.status === 'SKIPPED' && !ob.skippedBuddyIntroShown && state.chatMessages.filter(m => m.role === 'user').length === 0) {
      const intro: ChatMessage = { id: uid('msg'), role: 'assistant', createdAt: todayIso(), content: 'Hej! 👋 Jag såg att du valde att utforska Klirr själv. Det är helt okej 😊 Jag kan hjälpa dig bygga Budgeten när det passar dig. Vad vill du börja med?', actions: [
        { label: 'Lägg till inkomst', tab: 'income' }, { label: 'Lägg till Måsten', tab: 'musts' }, { label: 'Importera kontoutdrag', tab: 'import' }, { label: 'Skapa Rörlig Budget', tab: 'variablePlan' }, { label: 'Städa min Budget', message: 'Städa min Budget' },
      ] };
...
function SettingsView({ state, setState, onReset, loadDemo, restartOnboarding }: { state: AppState; setState: (s: AppState) => void; onReset: () => void; loadDemo: () => void; restartOnboarding: () => void }) {
  ...
  return <><PageTitle title="Inställningar" subtitle="Demo, integritet, export och återställning." />
    ...
    <div className="grid grid-2">
      ...
      <Card><h3>Kom igång igen</h3><p>Detta startar bara guiden igen. Din Budget och importerade data finns kvar.</p><div className="row"><button className="btn primary" onClick={() => setState({ ...state, onboardingCompleted: false, onboarding: { ...normalizeOnboardingState(state.onboarding), status: normalizeOnboardingState(state.onboarding).path === 'import' ? 'IMPORT_PATH' : 'MANUAL_PATH', started: true } })}>Fortsätt onboarding</button><button className="btn" onClick={restartOnboarding}>Starta onboarding igen</button></div></Card>
