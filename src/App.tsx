...
function DashboardView({ summary, budgetHealth, budgetCompletion, detection, visibleReviewCount, loadDemo, setTab, hasData, onboarding, dismissFirstRunGuide, onExport, onImproveBudgetHealth }: { summary: ReturnType<typeof calculateBudget>; budgetHealth: ReturnType<typeof calculateBudgetHealth>; budgetCompletion: ReturnType<typeof calculateBudgetCompletion>; detection: DetectionResult; visibleReviewCount: number; loadDemo: () => void; setTab: (t: TabId) => void; hasData: boolean; onboarding?: AppState['onboarding']; dismissFirstRunGuide: () => void; onExport: () => void; onImproveBudgetHealth: () => void }) {
  const [openSection, setOpenSection] = useState<string>('budget-health');
  ...
  const toggle = (id: string) => setOpenSection(current => current === id ? '' : id);
  const row = (id: string, title: string, value: string, subtitle: string, content: ReactNode, important = false) => <div className="home-row" key={id}>
    <button className="home-row-button" aria-expanded={openSection === id} aria-controls={`home-panel-${id}`} onClick={() => toggle(id)}>
      <span className="home-row-title">{important && <span className="home-warning-dot" aria-label="Viktigt">!</span>}{title}<small>{subtitle}</small></span>
      <span className="home-row-value mono">{value}</span><span className="home-chevron" aria-hidden="true">⌄</span>
    </button>
    {openSection === id && <div className="home-row-panel" id={`home-panel-${id}`}>{content}</div>}
  </div>;
  ...
  if (!hasData || budgetCompletion.percentage < 100) {
    return <><PageTitle title="Din Budget" subtitle="Vad livet kostar varje månad just nu — och hur hållbar Budgeten är." />
      ...
      <Card><h3>Bygg klart i lugn takt</h3><div className="home-list">
  ...
  return <>
    ...
    <Card><div className="home-list">
      ...
      {row('musts', 'Måsten / återkommande kostnader', fmt(summary.fixedTotal), summary.totalIncome > 0 ? `${pct(fixedPct)} av inkomsten` : 'Andel visas när inkomst finns', <><p>Måsten är kostnader som påverkar Budgeten framåt.</p><div className="progress"><div style={{ width: `${Math.min(100, fixedPct)}%` }} /></div>{summary.fixedItems.slice(0, 6).map(i => <div className="list-line" key={i.id}><span>{i.label}</span><b className="mono">{fmt(i.amount)}</b></div>)}<button className="btn small" onClick={() => setTab('musts')}>Visa Måsten</button></>, fixedPct > 65)}
