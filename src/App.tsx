...
import { budgetBuddyCheckupMessage, buildManualOnboardingPatch, getOnboardingStepNumber, normalizeOnboardingState, onboardingWarnings, type OnboardingStep } from './lib/onboarding';
...
function sourceLabel(source?: 'recurring' | 'manual' | 'variablePlan') {
  ...
  if (source === 'variablePlan') return 'Rörlig Budget';
  return source || '';
}
...
function OnboardingView({ initialState, state, setState, loadDemo, setTab }: { initialState: AppState; state: AppState; setState: (s: AppState) => void; loadDemo: () => void; setTab: (t: TabId) => void }) {
  ...
  function patchOnboarding(currentStep: OnboardingStep, patch: Partial<AppState['onboarding']> = {}) { setState({ ...state, onboarding: { ...onboarding, started: true, currentStep, ...patch } }); }
  function applyDraft(complete = false, nextTab: TabId = 'dashboard') {
    ...
    setState({ ...initialState, ...state, ...patch, onboardingCompleted: complete, onboarding: { ...onboarding, started: true, reviewCompleted: true, currentStep: complete ? 'finish' : 'summary' } });
    setTab(nextTab);
  }
...
function PlanView({ active, setActive, summary, scenarioSummary, detection, state, setState, setVariablePlan }: { active: PlanSection; setActive: (s: PlanSection) => void; summary: ReturnType<typeof calculateBudget>; scenarioSummary: ReturnType<typeof calculateBudget>; detection: DetectionResult; state: AppState; setState: (s: AppState) => void; setVariablePlan: (p: VariablePlanItem[]) => void }) {
  return <><PageTitle title="Plan" subtitle="Planera Budgeten framåt: Måsten, Rörlig Budget och scenarier." />
    <SectionTabs<PlanSection> active={active} onChange={setActive} items={[{ id: 'musts', label: 'Måsten' }, { id: 'variablePlan', label: 'Rörlig Budget' }, { id: 'scenarios', label: 'Scenarier' }]} />
...
Obs: Budget Buddy kunde inte svara via AI just nu. Inga ändringar gjordes — detta är ett lokalt tryggt fallback-svar.` }] });
  ...
  function actionSummary(action: BuddyProposedAction) {
    ...
    if (action.type === 'update_variable_plan') {
      ...
      return <div className="stack">{action.payload.mode === 'crisis' && <p className="hint">🛟 Tillfälligt krisläge: mat skyddas först; nöje, sparande och marginal sänks innan mat blir orimligt låg. Granska manuellt.</p>}<div className="list-line"><span>Nuvarande Rörlig Budget</span><b className="mono">{fmt(currentTotal)}</b></div><div className="list-line"><span>Föreslagen Rörlig Budget</span><b className="mono">{fmt(total)}</b></div>{action.payload.items.map(item => <div className="list-line" key={item.id || item.label}><span>{item.label}<br/><small>{item.category}</small></span><b className="mono">{fmt(item.amount)}</b></div>)}{Number.isFinite(action.payload.marginLeft) && <div className="list-line"><span>Uppskattat kvar efter plan</span><b className="mono">{fmt(action.payload.marginLeft || 0)}</b></div>}<p className="hint">Jag vill dubbelkolla innan jag ändrar något 🫶 Det här ersätter den nuvarande Rörliga Budgeten först när du bekräftar.</p>{action.payload.notes && <p className="hint">{action.payload.notes}</p>}{riskText && <span className="pill">{riskText}</span>}</div>;
    }
...
function MustsView({ summary, detection, state, setState }: { summary: ReturnType<typeof calculateBudget>; detection: DetectionResult; state: AppState; setState: (s: AppState) => void }) {
  ...
  return <><PageTitle title="Återkommande kostnader" subtitle="Fasta kostnader som måste betalas varje månad. Lägg manuella måsten här, inte under inkomster." />
    ...
    <Card><h3>Redigera måsten</h3><p className="hint">Här lägger du in fasta kostnader som inte syns i kontoutdraget, till exempel kontantbetalningar, delad hyra eller avtal du vill räkna med manuellt. Importerade måsten redigeras som overrides. Kontoutdraget ändras inte.</p><div className="stack">
      ...
      {recurringMusts.map(r => { const d = state.recurringDecisions[r.id]; const status = d?.status || (r.confidence >= 70 ? 'confirmed' : 'pending'); const counted = status !== 'rejected'; return <div className="edit-row" key={r.id}><input className="input" value={d?.labelOverride ?? r.label} onChange={e => updateRecurring(r.id, { labelOverride: e.target.value })} disabled={!counted} /><input className="input money-input" type="number" value={d?.monthlyAmountOverride ?? Math.round(r.monthlyAmount)} onChange={e => updateRecurring(r.id, { monthlyAmountOverride: Number(e.target.value) })} disabled={!counted} /><input className="input category-input" value={d?.category ?? r.category} onChange={e => updateRecurring(r.id, { category: e.target.value })} disabled={!counted} /><select className="select frequency-input" value={d?.frequencyOverride ?? r.frequency} onChange={e => updateRecurring(r.id, { frequencyOverride: e.target.value as Frequency })} disabled={!counted}><option value="monthly">Månad</option><option value="quarterly">Kvartal</option><option value="yearly">År</option><option value="irregular">Tillfällig</option></select><span className={`pill ${counted ? 'green' : 'danger'}`}>{counted ? 'Importerad/Återkommande' : 'Borträknad · räknas inte'}</span><small>{counted ? `Från bankimport · ${r.confidence}% säkerhet · ${frequencyLabel(d?.frequencyOverride ?? r.frequency)}` : 'Borträknad från Budgeten. Kontoutdraget finns kvar.'}</small>{counted ? <><button className="btn small" onClick={() => moveRecurringToVariable(r.id)}>Flytta till rörlig</button><button className="btn small danger" onClick={() => rejectRecurring(r.id)}>Räkna bort</button></> : <button className="btn small" onClick={() => updateRecurring(r.id, { status: 'confirmed' })}>Räkna med igen</button>}</div>; })}
      {!hasMusts && <Empty><b>Inga måsten ännu.</b><br/>Lägg till återkommande kostnader som hyra, el och försäkring så Klirr kan visa vad livet kostar varje månad. Nästa steg: lägg till en rad manuellt eller bekräfta importerade återkommande utgifter.</Empty>}
    </div></Card>
...
function ReviewView({ reviewItems, recurringDecisions, setRecurringDecisions, reviewDecisions, setReviewDecisions }: { reviewItems: DetectionResult['reviewItems']; recurringDecisions: Record<string, RecurringDecision>; setRecurringDecisions: (d: Record<string, RecurringDecision>) => void; reviewDecisions: Record<string, ReviewDecision>; setReviewDecisions: (d: Record<string, ReviewDecision>) => void }) {
  ...
  return <><PageTitle title="Oklara poster" subtitle="Saker som Klirr inte vill räkna in utan att du kontrollerar dem. Knapparna sparar beslutet direkt och posten försvinner från listan." />
    <div className="stack">{reviewItems.map(it => <Card className="warn" key={it.id}><div className="row" style={{ justifyContent: 'space-between' }}><div><b>{it.description}</b><br/><small>{it.date} · {reviewTypeLabel(it.type)}</small><p>{it.note}</p></div><b className="mono">{fmt(Math.abs(it.amount))}</b></div><div className="row"><button className="btn small primary" onClick={() => decide(it, 'confirmed')}>Räkna med</button><button className="btn small" onClick={() => decide(it, 'rejected')}>Räkna bort</button><span className="hint">Beslutet sparas och döljer posten.</span></div></Card>)}{!reviewItems.length && <Empty><b>Inget att granska just nu 🎉</b><br/>Oklara poster visas här när Klirr behöver din hjälp för att skydda Budgeten från dubbelräkning eller engångar. Nästa steg: kontrollera Återkommande eller Rörlig Budget.</Empty>}</div></>;
}
...
function SettingsView({ state, setState, onReset, loadDemo, restartOnboarding }: { state: AppState; setState: (s: AppState) => void; onReset: () => void; loadDemo: () => void; restartOnboarding: () => void }) {
  ...
  return <><PageTitle title="Inställningar" subtitle="Demo, integritet, export och återställning." />
    ...
    <div className="grid grid-2">
      ...
      <Card><h3>Onboarding</h3><p>Starta om guidningen för att bygga en första Budget manuellt.</p><button className="btn" onClick={restartOnboarding}>Starta onboarding igen</button></Card>
